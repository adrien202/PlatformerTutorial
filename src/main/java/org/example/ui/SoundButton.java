...
import static  org.example.utils.Constants.UI.PauseButtons.*;
    ...
    private void loadSoundImages() {
        ...
        for (int j = 0; j < soundImages.length; j++) {
            for(int i = 0; i < soundImages[j].length; i++) {
                soundImages[j][i] = img.getSubimage(i * SOUND_SIZE_DEFAULT, j * SOUND_SIZE_DEFAULT,SOUND_SIZE_DEFAULT, SOUND_SIZE_DEFAULT);
            }
        }
    }
